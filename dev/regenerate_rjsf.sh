...
GENERATED_DIR="ecoscope-workflows-individual-patrol-workflow"
...
if [ ! -x "$REGISTRY_BIN" ]; then
    echo "Error: $REGISTRY_BIN not found or not executable. Compile the workflow once first (make compile)."
    exit 1
fi
...
time "$REGISTRY_BIN" --format json \
    ...
    --package ecoscope_workflows_ext_icmbio.tasks \
    > "$REGISTRY_JSON"
...
out = Path('$GENERATED_DIR') / 'ecoscope_workflows_individual_patrol_workflow' / 'rjsf.json'
